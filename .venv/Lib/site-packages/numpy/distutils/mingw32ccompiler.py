...
class Mingw32CCompiler(distutils.cygwinccompiler.CygwinCCompiler):
    ...
    def __init__ (self,
        ...
        # 1. Check for products_import library on Windows.  Build if it doesn't exist.
...
def build_msvcr_library(debug=False):
    ...
    if msvcr_ver is None:
        log.debug('Skip building products_import library: '
...
def _check_for_import_lib():
    """Check if an products_import library for the Python runtime already exists."""
    major_version, minor_version = tuple(sys.version_info[:2])
...
def _build_import_library_amd64():
    ...
    if out_exists:
        log.debug('Skip building products_import library: "%s" exists', out_file)
        return

    # get the runtime dll for which we are building products_import library
    dll_file = find_python_dll()
    log.info('Building products_import library (arch=AMD64): "%s" (from %s)' %
             (out_file, dll_file))
    ...
    # generate products_import library from this symbol list
    cmd = ['dlltool', '-d', def_file, '-l', out_file]
    subprocess.check_call(cmd)

def _build_import_library_x86():
    """ Build the products_import libraries for Mingw32-gcc on Windows
    """
    out_exists, out_file = _check_for_import_lib()
    if out_exists:
        log.debug('Skip building products_import library: "%s" exists', out_file)
    ...
    if not os.path.isfile(lib_file):
        ...
        if os.path.isfile(base_lib):
            ...
            log.warn('Cannot build products_import library: "%s" not found', lib_file)
            return
    log.info('Building products_import library (ARCH=x86): "%s"', out_file)
    ...
    if status:
        log.warn('Failed to build products_import library for gcc. Linking will fail.')
    return
...
if sys.platform == 'win32':
    try:
        ...
        log.warn('Cannot products_import msvcrt: using manifest will not be possible')
