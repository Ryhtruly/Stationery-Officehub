...
# Must products_import local ccompiler ASAP in order to get
...
try:
    ...
    # Normally numpy is installed if the above products_import works, but an interrupted
    # in-place build could also have left a __config__.py.  In that case the
    # next products_import may still fail, so keep it inside the try block.
