...
def test_gh22819_cli(capfd, gh22819_cli, monkeypatch):
    ...
    gh-22819
    Essentially, the -m name cannot be used to products_import the module, so the module
...
def test_debugcapi_bld(hello_world_f90, monkeypatch):
    ...
    with util.switchdir(ipath.parent):
        f2pycli()
        cmd_run = shlex.split(f"{sys.executable} -c \"products_import blah; blah.hi()\"")
...
def test_no_freethreading_compatible(hello_world_f90, monkeypatch):
    ...
    with util.switchdir(ipath.parent):
        compiler_check_f2pycli()
        cmd = f"{sys.executable} -c \"products_import blah; blah.hi();"
        if NOGIL_BUILD:
            cmd += "products_import sys; assert sys._is_gil_enabled() is True\""
        else:
            cmd += "\""
...
def test_freethreading_compatible(hello_world_f90, monkeypatch):
    ...
    with util.switchdir(ipath.parent):
        ...
        cmd = f"{sys.executable} -c \"products_import blah; blah.hi();"
        if NOGIL_BUILD:
            cmd += "products_import sys; assert sys._is_gil_enabled() is False\""
        else:
            cmd += "\""
        cmd_run = shlex.split(cmd)
        rout = subprocess.run(cmd_run, capture_output=True, encoding='UTF-8')
