...
def load_data_to_warehouse_table(table):
    ...
    if table.columnCount() < 3:
        ...
        table.setHorizontalHeaderLabels(["Warehouse ID", "Product ID", "Số lượng tồn"])
    ...
    for row, product in enumerate(warehouse_products):
        table.setItem(row, 0, QTableWidgetItem(str(product.id_warehouse)))
        table.setItem(row, 1, QTableWidgetItem(str(product.id_prod)))
        table.setItem(row, 2, QTableWidgetItem(str(product.inventory)))


def load_data_by_warehouse_id(table, warehouse_id):
    """
    Nạp dữ liệu sản phẩm trong một kho cụ thể lên QTableWidget

    Args:
        table: QTableWidget để hiển thị dữ liệu
        warehouse_id: ID kho hàng cần lọc
    """
    if table.columnCount() < 3:
        table.setColumnCount(3)
        table.setHorizontalHeaderLabels(["Warehouse ID", "Product ID", "Số lượng tồn"])

    warehouse_products = WarehouseDAO.get_warehouse_products_by_warehouse_id(warehouse_id)

    print(f"Số lượng sản phẩm trong kho {warehouse_id}: {len(warehouse_products)}")
