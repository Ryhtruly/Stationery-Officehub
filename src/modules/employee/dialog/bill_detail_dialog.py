...
class BillDetailDialog(QDialog):
    def __init__(self, bill_id, parent=None):
        ...
        self.detail_table.verticalHeader().setVisible(False)  # Ẩn cột số thứ tự
        # Áp dụng stylesheet cho bảng
        ...
        print("Đã thêm QTableWidget vào layout")  # Log
        ...
        # Hiển thị tổng tiền (di chuyển xuống sau bảng chi tiết)
        ...
        # Thêm dòng "Cảm ơn quý khách đã mua hàng" trước các nút
    ...
    def safe_str(self, value):
        ...
        if value is None:
            return ""
        try:
            # Đảm bảo giá trị đã là chuỗi trước khi encode
            str_value = str(value)
            # Encode và decode để xử lý ký tự tiếng Việt
            return str_value.encode('utf-8', errors='ignore').decode('utf-8', errors='ignore')
        except Exception as e:
            print(f"Lỗi xử lý chuỗi: {value}, lỗi: {str(e)}")
            return str(value)  # Trả về chuỗi gốc nếu không xử lý được
    ...
    def load_bill_info(self):
        ...
        # Lưu thông tin hóa đơn để sử dụng khi xuất PDF
        self.bill_info = bill
        ...
        if bill:
            ...
            try:
                ...
                print("Hiển thị Tổng tiền")  # Log
                total = bill.get('total', 0)  # Đảm bảo total không bị None
                print(f"Tổng tiền từ cơ sở dữ liệu: {total}")  # Log giá trị tổng tiền
                self.total_label.setText(f"Tổng tiền: {self.format_currency(total)}")
                print("Hoàn tất hiển thị thông tin hóa đơn")  # Log
            except Exception as e:
